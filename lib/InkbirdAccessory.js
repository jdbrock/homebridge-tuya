...
class SimpleHeaterAccessory extends BaseAccessory {
    ...
    _registerCharacteristics(dps) {
        ...
        this.dpActive = this._getCustomDP(this.device.context.dpActive) || '115';
        this.dpDesiredTemperature = this._getCustomDP(this.device.context.dpDesiredTemperature) || '106';
        this.dpCurrentTemperature = this._getCustomDP(this.device.context.dpCurrentTemperature) || '104';
        ...
        const characteristicHeatingThresholdTemperature = service.getCharacteristic(Characteristic.HeatingThresholdTemperature)
            .setProps({
                minValue: this.device.context.minTemperature || 1,
    ...
    _getActive(dp) {
        ...
        return parseInt(dp) === 3 ? Characteristic.Active.ACTIVE : Characteristic.Active.INACTIVE;
    }
    ...
    _getCurrentHeaterCoolerState(dps) {
        ...
        const state = parseInt(dps[this.dpActive]);
